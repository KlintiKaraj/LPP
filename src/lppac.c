/*
** $Id: LPPc.c $
** LPP compiler (saves bytecodes to files; also lists bytecodes)
...
#define PROGNAME	"LPPc"		/* default program name */
...
static const Proto* combine(LUA_State* L, int n)
{
 ...
 else
 {
  Proto* f;
  int i=n;
  if (LPP_load(L,reader,&i,"=(" PROGNAME ")",NULL)!=LUA_OK) fatal(LPP_tostring(L,-1));
  f=toproto(L,-1);
  for (i=0; i<n; i++)
  {
...
static int pmain(LUA_State* L)
{
 ...
 int i;
 tmname=G(L)->tmname;
 if (!LPP_checkstack(L,argc)) fatal("too many input files");
 for (i=0; i<argc; i++)
 {
  const char* filename=IS("-") ? NULL : argv[i];
  if (LPPL_loadfile(L,filename)!=LUA_OK) fatal(LPP_tostring(L,-1));
 }
 f=combine(L,argc);
...
int main(int argc, char* argv[])
{
 ...
 LPP_pushcfunction(L,&pmain);
 LPP_pushinteger(L,argc);
 LPP_pushlightuserdata(L,argv);
 if (LPP_pcall(L,2,0,0)!=LUA_OK) fatal(LPP_tostring(L,-1));
 LPP_close(L);
 return EXIT_SUCCESS;
}
...
static void PrintCode(const Proto* f)
{
 ...
 for (pc=0; pc<n; pc++)
 {
  ...
  int isk=GETARG_k(i);
  int line=LPPG_getfuncline(f,pc);
  printf("\t%d\t",pc+1);
